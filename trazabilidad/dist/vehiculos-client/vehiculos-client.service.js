...
let VehiculosClientService = VehiculosClientService_1 = class VehiculosClientService {
    ...
    async getVehiculo(vehicleId) {
        try {
            ...
            this.logger.warn(`No se pudo obtener el vehículo ${vehicleId}: ${error.message}`);
            return null;
        }
    }
    async checkHealth() {
        try {
            await (0, rxjs_1.firstValueFrom)(this.httpService.get(`${this.vehiculosBaseUrl}/vehiculos`));
            return true;
        }
        catch {
            return false;
