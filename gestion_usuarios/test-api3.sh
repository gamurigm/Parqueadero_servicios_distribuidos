...
# Start the app in background with setsid to detach from shell
setsid node dist/src/main.js > /tmp/app.log 2>&1 &
APP_PID=$!
...
# Test create usuario
echo "POST /usuario"
curl -v -X POST http://localhost:3000/usuario \
  -H "Content-Type: application/json" \
  -d '{
    "id": "test-uuid-1",
    "username": "testuser",
...
echo "GET /usuario"
curl -v http://localhost:3000/usuario
echo ""

# Test create role
echo "POST /roles"
curl -v -X POST http://localhost:3000/roles \
  -H "Content-Type: application/json" \
  -d '{
    "nombre": "admin",
...
# Test get all roles
echo "GET /roles"
curl -v http://localhost:3000/roles
echo ""
