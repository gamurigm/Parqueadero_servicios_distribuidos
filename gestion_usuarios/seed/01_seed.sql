...
-- ── 2. PERSONAS ─────────────────────────────────────────────
INSERT INTO personas (id, active, first_name, middle_name, last_name, dni, email, phone, address, nationality, tipo, created_at, updated_at)
SELECT gen_random_uuid(), true, v.fn, v.mn, v.ln, v.dni, v.email, v.phone, v.addr, 'ecuatoriana', v.tipo, NOW(), NOW()
FROM (VALUES
...
SELECT p.id, v.username, v.hash, true, NOW(), NOW()
FROM personas p
JOIN (VALUES
